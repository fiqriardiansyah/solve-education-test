...
const ButtonAnswerAnimated = ({ option, rightOption, pickOption, ...props }: ButtonAnswerType) => {
    ...
    return (
        <AnimatePresence mode="sync">
            {(pickOption && pickOption !== option) ? null : (
                <motion.div exit={{ y: '100px', opacity: 0, zIndex: 0, height: 0 }} transition={{ duration: .7 }}>
                    <ButtonAnswer
                        variant={pickOption ? "contained" : "outlined"}
                        color={color}
                        sx={{ textTransform: "capitalize", color: pickOption ? "white" : blue[300] }}
                        {...props}
                    >
                        {option}
                    </ButtonAnswer>
